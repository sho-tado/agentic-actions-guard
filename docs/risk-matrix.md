...
| `UNTRUSTED_INPUT_TO_AGENT` | Untrusted event text or workflow input into AI automation | Can an issue, PR, comment, discussion, discussion answer, review, commit, branch, dispatch input, reusable workflow input, dispatch payload, serialized event object, or event payload file shape the agent prompt? | Treat the text or input as hostile; isolate, summarize, or constrain it before agent use. |
