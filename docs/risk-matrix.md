...
| `UNTRUSTED_INPUT_TO_AGENT` | Untrusted event text or workflow input into AI automation | Can an issue, PR, comment, review, commit, branch, dispatch input, reusable workflow input, or dispatch payload shape the agent prompt? | Treat the text or input as hostile; isolate, summarize, or constrain it before agent use. |
