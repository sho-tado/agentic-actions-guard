...
def test_adoption_recipes_are_linked_from_entrypoints() -> None:
    ...
    assert "sho-tado/agentic-actions-guard@v1.10.38" in recipes
...
def test_risk_matrix_documents_all_rule_ids_and_is_linked() -> None:
    ...
    reference = (ROOT / "docs" / "rule-reference.md").read_text(encoding="utf-8")
    playbook = (ROOT / "docs" / "maintainer-review-playbook.md").read_text(encoding="utf-8")
    threat_model = (ROOT / "docs" / "ai-actions-threat-model.md").read_text(encoding="utf-8")
    checklist = (ROOT / "docs" / "ai-github-actions-safety-checklist.md").read_text(encoding="utf-8")

    for rule_id in RULE_METADATA:
        assert f"`{rule_id}`" in matrix
    assert "discussion" in matrix
    assert "`github.event.discussion.body`" in threat_model
    assert "`github.event.discussion_comment.body`" in threat_model
    assert "`github.event.answer.body`" in threat_model
    assert "toJson(github.event.issue)" in threat_model
    assert "github.event_path" in threat_model
    assert "discussion titles, bodies, comments, and accepted answers" in checklist
    assert "serialized event objects" in checklist
    assert "event payload files" in checklist
...
def test_ai_workflow_risk_gallery_is_linked_from_entrypoints() -> None:
    gallery = (ROOT / "docs" / "ai-workflow-risk-gallery.md").read_text(encoding="utf-8")
    readme = (ROOT / "README.md").read_text(encoding="utf-8")

    assert "AI Workflow Risk Gallery" in gallery
    assert "The Friendly Issue Triage Bot" in gallery
    assert "The Env Variable That Hides The Boundary" in gallery
    assert "The Model Output That Becomes A Command" in gallery
    assert "The Pull Request Target Trap" in gallery
    assert "The Helpful Auto-Fix That Pushes" in gallery
    assert "The Workflow Run Handoff" in gallery
    assert "Quick Share Copy" in gallery
    assert "docs/ai-workflow-risk-gallery.md" in readme
...
def test_output_schema_contract_is_linked_from_entrypoints() -> None:
    schema = (ROOT / "docs" / "output-schema.md").read_text(encoding="utf-8")
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
    lifecycle = (ROOT / "docs" / "finding-lifecycle.md").read_text(encoding="utf-8")
    code_scanning = (ROOT / "docs" / "github-code-scanning.md").read_text(encoding="utf-8")
    reference = (ROOT / "docs" / "rule-reference.md").read_text(encoding="utf-8")

    assert "Output Schema Contract" in schema
    assert "JSON Output" in schema
    assert "Finding Object" in schema
    assert "Suppression Object" in schema
    assert "SARIF Output" in schema
    assert "runs[0].properties.suppressions" in schema
    assert "removalCondition" in schema
    assert "removal_condition" in schema
    assert "docs/output-schema.md" in readme
    assert "output-schema.md" in lifecycle
    assert "output-schema.md" in code_scanning
    assert "output-schema.md" in reference


def test_scanner_precision_notes_are_linked_from_entrypoints() -> None:
    precision = (ROOT / "docs" / "scanner-precision.md").read_text(encoding="utf-8")
    readme = (ROOT / "README.md").read_text(encoding="utf-8")
...
def test_step_summary_example_is_linked_from_entrypoints() -> None:
    ...
    assert "Agentic Actions Guard Summary" in step_summary
    assert "sho-tado/agentic-actions-guard@v1.10.38" in step_summary
